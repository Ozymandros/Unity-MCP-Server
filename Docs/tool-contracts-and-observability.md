...
The **`unity_create_prototype_recipe`** MCP tool is a Phase 2/3 project bootstrapper with the same JSON result shape. It resolves the project (existing `projectPath` or scaffold via `projectName` + `outputRoot`), then runs: install_packages, configure_urp, create_default_scene; optionally (when flags are true) create_ui_canvas/create_ui_layout (main menu), configure_navmesh, create_waypoint_graph, create_input_actions, create_basic_animator, create_advanced_animator, create_timeline, create_vfx_asset, create_physics_setup; and always validate_import last.

- **Parameters**: `projectName`, `outputRoot`, `projectPath`, `sceneName` (default `"MainScene"`), optional `packagesJson` (JSON array of package IDs; when null/empty, default URP + TextMeshPro list is used), `includeNav`, `includeInput`, `includeAnimator`, `includeAdvancedAnimator`, `includeTimeline`, `includeVfx`, `includePhysics`, `includeMainMenu` (all optional booleans, default false).
